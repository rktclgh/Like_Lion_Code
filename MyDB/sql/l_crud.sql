...
SET NULL | 부모 테이블의 데이터가 삭제/수정되면 자식테이블의 컬럼값만 null
NO ACTION | restrict 와 같다
SET DEFAULT | 부모 테이블의 데이터가ㅣ 삭제/수정되면 자식테이블의 컬럼값은 기본값으로 대체된다.
...
- 고객 정보가 삭제되면 그 고객의 주문도 함께 삭제된다. cascade
- 고객의 id가 변경되면 주문정보의 고객 id도 함께 변경된다. cascade

alter talbe emp
add constraint fk_emp_dept
            foreign key(deptno) references (debtno)
                    on delete no action on update no action;
    - on delete: dept 테이블에서 deptno값을 삭제 하려고 할떄,
                deptno를 사용하는 emp테이블의 데이터가 있으면 삭제를 허용하지 않겟다.
    -on update no action: dept테이블에서 deptno 값을 수정 하려고 할떄,.
                deptno를 사용하는 emp테이블의 데이터가 있으면 수정을 허용하지 않겠다.
...
show table status like 'TEMP_KING';
...
-- Q13) 두개의 테이블 제약조건을 확인하자.

-- Q14) 두개의 테이블 제약조건을 삭제하자.
