...
-- Q14)  사원테이블에서 부서 번호와 각 부서 (DEPTNO) 직원의 평균 재직 기간(년)을 출력 하자.  
 ...
 -- 표본분산 var_samp(expr)[over_clause] n-1로 나누는 방식으로 게산된다.
 -- 모집단분산 var_pop(expr)[over_clause] n로 나누는 방식으로 게산된다.
 -- case1
...
-- case 2. 윈도우 함수를 활용해서 부서별로 급여의 표본분산을 출력하자.
    select emp.deptno, var_samp(sal) over (partition by deptno) as "res"
    from emp;

select emp.deptno, var_pop(sal) over (partition by deptno) as "res"
    from emp;
