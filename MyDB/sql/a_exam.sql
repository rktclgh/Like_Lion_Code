...
#MYSQL
    -- Insert into 테이블명 values(값); //값의 순서가 타입에 맞아야 한다.
            -- ex) insert into 테이블 values(100, a, null)
            -- ex) insert into 테이블 values(100, null, a); number, varchar, varchar

    -- Insert into 테이블명 (컬럼 명)values(값);
    -- Insert into 테이블명 (이름)values(값);
...
-- default 는 insert 하자 얺는 속성이 있다면 기본값을 입력된다.
CREATE TABLE t1 (
  -- literal defaults -> 크기를 지정, 수치를 크기 NUMBER(전체 크기. 소수자리)
    -- UUID_TO_BIN(UUID()) VER.8.0 -> 인덱스 성능 향상
    -- UUID()는 보통 32개 자리를 사용 -> BINARY(16)자리로 변경해서 사용
  i INT         DEFAULT 0,
  c VARCHAR(10) DEFAULT 'ABCD',
  -- expression defaults
  f FLOAT       DEFAULT (RAND() * RAND()),
  b BINARY(16)  DEFAULT (UUID_TO_BIN(UUID())), -- UUID() MYSQL 에서 고유 id 생성
  d DATE        DEFAULT (CURRENT_DATE + INTERVAL 1 YEAR),
  p POINT       DEFAULT (Point(0,0)), -- 공간 데이터 좌표
...
select json_object('이름', emp.ename, '직업', job, '봉급', sal) as 사원정보
from my_emp.emp
where deptno=10;
-- JSON_ARRAY() : 배열 생성
-- JSON_OBJECT() : 객체 생성(KEY-VALUE)
-- JSON_ARRAY GG() : group by 한 후 결과를 jSON 배열로 리턴
