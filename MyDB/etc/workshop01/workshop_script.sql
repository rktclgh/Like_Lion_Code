...
from BOOK
where REG_DATE > 20200620;

-- Q3.카테고리가 'B001'이고 대출 횟수가 8회인 도서의 식별번호, isbn, 제목, 대출횟수 정보를 조회하세요.

select bk_idx, isbn, title, RENT_CNT
from BOOK
where CATEGORY = 'B001' and RENT_CNT = 8 ;

-- Q4.저자가 '혜민'인 도서들의 도서들의 식별번호, isbn, 제목, 저자 정보를 조회하세요.
select bk_idx, isbn, title, author
from book
where author like '혜민%';

-- Q5. 도서 제목에 '소설'이 포함된 도서들의 제목 정보를 조회하세요.
select BOOK.TITLE
from BOOK
where title like '%소설%';

-- Q6. 등록일이 2020년 4월 15일 이전이고 대출 횟수가 10회 이하인 도서들의 식별번호, isbn, 제목,저자, 등록일, 대출횟수 정보를 조회하세요.
select bk_idx, isbn, title, RENT_CNT
from BOOK
where REG_DATE <20200415 and RENT_CNT<10;


-- Q7.도서 제목이 '나는'으로 시작하는 도서들의 식별번호, isbn, 제목, 저자, 등록일, 대출횟수 정보를 조회하세요.
select bk_idx, isbn, title, author,RENT_CNT
from BOOK
where title like '나는%';

-- Q8. 도서 카테고리별 평균 대출건수와 총 대출건수 조회하세요.

select BOOK.CATEGORY,avg(BOOK.RENT_CNT) as '평균 대출건수', sum(BOOK.RENT_CNT) as '총 대출건수'
from BOOK
group by CATEGORY;

-- Q9.가장 대출 횟수가 많은 도서 5권의 식별번호, isbn, 제목, 저자, 등록일, 대출횟수 정보를 조회하세요.

select bk_idx, isbn, title, author, BOOK.RENT_CNT
from BOOK
order by RENT_CNT desc
limit 5;

-- Q10 도서 카테고리가 B002인 도서의 식별번호, isbn, 카테고리, 제목, 저자, 등록일정보를 등록일자 순으로 내림차순으로 조회하세요.
...
-- Q11. 회원 등급이 ROLE_USER 인 회원을 이름으로 오름차순 정렬하여 조회하세요.
...
-- Q12. 등급이 'ROLE_ADMIN'인 사용자 중 퇴사하지 않은 사용자만 조회하세요.
select user_id, password, email, grade, tell, is_leave
from MEMBER
where IS_LEAVE = 0 and GRADE = 'role_admin';

-- Q13. 등급 별 사용자 수를 조회하세요.
select GRADE, count(*)
from MEMBER
group by grade;

-- Q14. 등급 별 사용자 수를 조회하세요.
select user_id, password, email, grade, tell, is_leave
from MEMBER
where GRADE  = 'role_user' and tell like '%9';

-- Q15. 퇴사하지 않은 사용자 중에서 등급이 'ROLE_ ADMIN' 인 사용자의 이메일 조회 (알파벳 순)
select email
from MEMBER
where IS_LEAVE = 0 and grade = 'role_admin';
...
from MEMBER
WHERE IS_LEAVE = 0
  AND REPLACE(tell, '-', '') REGEXP '^...2' and grade = 'role_user' and USER_ID regexp '^E'
...
show databases ;
use workshop01;
select bk_idx, title, book_amt, rank()over(order by BOOK_AMT) as 'price_rank'
from BOOK

-- Q2. 카테고리별로 대여 횟수 순위를 매겨 책 제목, 대여 횟수와 함께 출력하시오.
select bk_idx, CATEGORY,title, RENT_CNT, rank()over(order by RENT_CNT) as 'rent_rank'
from BOOK;
-- Q3. 등록일 기준으로 가장 오래된 도서와 가장 최근 도서 제목을 각각 출력하시오.     (FIRST_VALUE, LAST_VALUE 함수 활용)

select BOOK.BK_IDX, title, BOOK.REG_DATE,
       first_value(TITLE) over (order by BOOK.REG_DATE) as 'oldest_book',
       last_value(title) over (order by REG_DATE)as 'newest_book'
from BOOK;

-- Q4. 카테고리별 평균 가격과 각 도서의 가격 차이를 계산하여 책 제목과 함께 출력하시오.(AVG 함수 활용)
select BOOK.BK_IDX, BOOK.CATEGORY, title, BOOK.BOOK_AMT, avg(BOOK.BOOK_AMT) over (partition by BOOK.CATEGORY) as avg_price,
       BOOK.BOOK_AMT - avg(BOOK.BOOK_AMT) over(partition by BOOK.CATEGORY) as price_diff
from BOOK;

-- Q5. 카테고리별 총 대여 횟수를 계산하여 책 제목과 함께 출력하시오. (SUM 함수 활용)

select BOOK.BK_IDX, BOOK.CATEGORY, title, BOOK.RENT_CNT, sum(BOOK.RENT_CNT) over (partition by BOOK.CATEGORY) as total_rent
from book;
...
# select * from book;
# select title, BOOK.BOOK_AMT,
# from book;

-- Q11) 등록일로부터 현재까지 경과한 날짜를 계산하여 책 제목과 함께 출력하시오. (DATEDIFF함수 활용)

select title, BOOK.REG_DATE, datediff(now(),BOOK.REG_DATE)
from BOOK;

-- Q12) 대여 횟수의 최대값과 각 도서의 대여 횟수 차이를 계산하여 책 제목과 함께 출력하시오.(MAX 함수 활용)

select title, BOOK.RENT_CNT, max(BOOK.RENT_CNT)over ()-RENT_CNT as diff_max_rent
from book;
...
SELECT TITLE
FROM BOOK
WHERE LOCATE('SQL', UPPER(TITLE)) > 0;
...
select title, BOOK.REG_DATE, month(BOOK.REG_DATE) as reg_month
from BOOK

-- Q18) 도서 제목과 저자명을 연결하여 출력하시오. (CONCAT 함수 활용)

select title, author, concat(title, author)
from book;
...
select title, replace(title, ' ', '') as title_no_space
from BOOK

-- Q23) 도서 가격의 제곱값을 계산하여 책 제목과 함께 출력하시오. (POWER 함수 활용)

select title, BOOK.BOOK_AMT, power(BOOK.BOOK_AMT,2) as price_squared
from book;

-- Q24) 등록일의 요일을 계산하여 책 제목과 함께 출력하시오. (DAYNAME 함수 활용)

select title, BOOK.REG_DATE, dayname(BOOK.REG_DATE)
from book;

-- Q25) 대여 횟수를 기준으로 다음 도서의 대여 횟수를 함께 출력하시오. (LEAD 함수 활용)
select title, BOOK.RENT_CNT, lead(BOOK.RENT_CNT) over (order by BOOK.RENT_CNT) as next_Rent_cnt
from book;
...
SELECT
    TITLE,
    SUBSTRING(REGEXP_REPLACE(TITLE, '\\(.*\\)', '') FROM 1 FOR 5) AS short_title
FROM BOOK;


-- Q28) 등록일 기준으로 누적 도서 수를 계산하여 책 제목과 함께 출력하시오. (COUNT + 윈도우 함수 활용)

select title, reg_date, count(*) over (order by BOOK.REG_DATE) as cumulative_book_count
from book;


-- q30 대여 횟수 순위를 매겨 동일 값이면 같은 순위로 책 제목과 함께 출력하시오. (RANK 함
select title, BOOK.RENT_CNT, rank() over(order by RENT_CNT desc) as rent_rank
from book;